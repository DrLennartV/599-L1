...
class CNN_Data(Dataset):
    ...
    def __init__(self, csv_dir):
        ...
        self.name_ls = df['filename'].values
        self.ad_ls = df['AD'].values
        self.mri_ls = []
        for fn in self.name_ls:
            mri = np.load(f'{MRI_FOLDER}/{fn}')
            mri = mri[np.newaxis, :, :, :]
            self.mri_ls.append(mri)


    # Returns total number of data samples
    def __len__(self):
        # YOUR CODE HERE 
        return len(self.ad_ls)
    ...
    def __getitem__(self, idx):
        ...
        '''
        # YOUR CODE HERE 
        return self.mri_ls[idx], self.name_ls[idx], self.ad_ls[idx]
...
def split_csv(csv_file, output_folder='./ADNI3', random_seed = 1051):
    ...
    '''
    # YOUR CODE HERE 
    file_fm = os.listdir(MRI_FOLDER)

    file_dict = {}
    file_ls, ad_ls = read_csv(csv_file)
    for k, v in zip(file_ls, ad_ls):
        file_dict[k] = v

    ad_fm = []
    for f in file_fm:
        ad_fm.append(file_dict[f])

    zp = list(zip(file_fm, ad_fm))
    random.seed(random_seed)
    random.shuffle(zp)
    file_fm[:], ad_fm[:] = zip(*zp)

    test_file_ls = file_fm[:6]
    test_ad_ls = ad_fm[:6]
    bg_file_ls = file_fm[6:]
    bg_ad_ls = ad_fm[6:]

    test_df = pd.DataFrame(data={'filename': test_file_ls, 'AD': test_ad_ls})
    bg_df = pd.DataFrame(data={'filename': bg_file_ls, 'AD': bg_ad_ls})
    test_df.to_csv(f"{output_folder}/test_data.csv", index=False)
    bg_df.to_csv(f"{output_folder}/bg_data.csv", index=False)
...
def read_csv(filename):
    ...
    file_ls = df['filename'].values
    ad_ls = df['AD'].values
    return file_ls, ad_ls
